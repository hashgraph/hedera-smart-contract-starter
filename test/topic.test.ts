...
describe("Topic", function () {
  ...
  beforeEach(async function () {
    ...
    Topic = await upgrades.deployProxy(TopicFactory, ["MyTopicId"], { initializer: 'initialize' });
  ...
  it("should return the correct topicId", async function () {
    expect(await (Topic as unknown as TopicContract).getTopicId()).to.equal("MyTopicId");
  });

  it("should return the correct topicId for version 2", async function () {
    expect(await (TopicV2 as unknown as TopicContract).getTopicId()).to.equal("MyTopicId");
  });

  it("should set and return the correct message for version 2", async function () {
    await (TopicV2 as unknown as TopicContract).setMessage("MyMessage");
    expect(await (TopicV2 as unknown as TopicContract).getMessage()).to.equal("MyMessage");
  });
});
